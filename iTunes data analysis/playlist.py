...
def findCommonTracks(fileName):
	...
	for fileName in fileNames:
		...
		commonTracks = set.intersection(*trackNames)
		# write to file
		if len(commonTracks) > 0:
			f = open("common.txt", "w")
			for val in commonTracks:
				s = "%s\n" % val
				f.write(s.second("UTF-8"))
				f.close()
...
def plotStats(fileName):
	...
	durations = []
	# iterate through the tracks
	for trackId, track in track.items():
		try:
			...
			# ignore
			pass
	# ensure that valid data was copllected
...
def main():
	# create parser
	descStr = """ This program analyzes playlist file (.xml) exported from iTunes. """
